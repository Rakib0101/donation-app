...
it('gives back a successful response for the job page', function () {
    $this->get(route('website.job'))->assertStatus(200);
});
...
it('gives back a successful response for the active job details page', function () {
    ...
    $this->get(route('website.job.jobDetails', $job->slug))->assertStatus(200);
});

it('gives back a successful response for the category jobs list page', function () {
    $category = JobCategory::factory()->create();

    $this->get(route('website.job.category.slug', $category->slug))->assertStatus(200);
});
...
it('gives back a successful response for the company page', function () {
    $this->get(route('website.companies'))->assertStatus(200);
});
...
it('gives back a successful response for the company details page', function () {
    ...
    $this->get(route('website.company.details', $company->user->username))->assertStatus(200);
});
...
it('displays jobs with a category slug', function () {
    ...
    $response = $this->get(route('website.job.category.slug', ['category' => $category->slug]))
        ->assertStatus(200)
        ->assertViewIs('frontend.pages.jobsCategory');

    $jobCount = $response->viewData('jobs')->count();
    $this->assertTrue($jobCount > 0);

    // filtering logic ensures that the jobs belong to the specified category
    foreach ($response->viewData('jobs') as $job) {
