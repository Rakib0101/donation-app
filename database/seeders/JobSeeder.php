...
class JobSeeder extends Seeder
{
    ...
    public function run()
    {
        $company_list = \File::json(resource_path('backend/dummy-data/jobs.json'));
        ...
        for ($i = 0; $i < count($company_list); $i++) {
            ...
            $state_id = State::where('country_id', $country_id)->inRandomOrder()->value('id');
            $city_id = City::where('state_id', $state_id)->inRandomOrder()->value('id');

            $job_data[] = [
                'title' => $company_list[$i]['title'],
                'slug' => Str::slug($company_list[$i]['title']).'_'.time().'_'.rand(1111111111, 9999999999),
                ...
                'vacancies' => fake()->randomElement(['1-2', '2-3', '3-5', '5-10', '10-20']),
                'min_salary' => $company_list[$i]['min_salary'],
                'max_salary' => $company_list[$i]['max_salary'],
                'salary_mode' => Arr::random(['range', 'custom']),
                'custom_salary' => 'Competitive',
                'deadline' => fake()->dateTimeBetween('now', '+07 days'),
                'is_remote' => $company_list[$i]['is_remote'],
                'status' => 'active',
                ...
                'description' => $this->getDescription(
                    $company_list[$i]['title'],
                    $company,
                    $job_type,
                    $company_list[$i]['min_salary'],
                    $company_list[$i]['max_salary']
                ),
            ];
        }
    ...
    {
        $job_list = \File::json(resource_path('backend/dummy-data/jobs.json'));

        $countries = Country::inRandomOrder()->limit(20)->get();
        foreach ($countries as $key => $country) {
            ...
            for ($i = 0; $i < 5; $i++) {
                ...
                $product1 = [
                    ...
                    'urgent' => Arr::random([false, true, false, true, false, true]),
